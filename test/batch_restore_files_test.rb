...
class TestBatchRestoreFiles < Minitest::Test
  ...
  def test_batch_restore
    ...
    files_ret = [{ 'id' => '1', 'name' => 'test', 'size' => 123_456, 'md5_sum' => '12345678901234567890123456789012', 'cfs_directory_id' => '3' },
                 { 'id' => '2', 'name' => 'test1', 'size' => 234_567, 'md5_sum' => '23456789012345678901234567890123', 'cfs_directory_id' => '6' }]
  ...
  def test_get_batch_restore_from_list
    ...
    files_ret = [{ 'id' => id1.to_s, 'name' => 'test', 'size' => 123_456, 'md5_sum' => '12345678901234567890123456789012', 'cfs_directory_id' => '3' }]
    @mock_db.expect(:exec_params, files_ret, [sql, [value: id1.to_s]])

    id2 = 2
    sql = 'SELECT * FROM cfs_files WHERE id=$1'
    files_ret = [{ 'id' => id2.to_s, 'name' => 'test1', 'size' => 234_567, 'md5_sum' => '23456789012345678901234567890123', 'cfs_directory_id' => '6' }]
  ...
  def test_get_max_id
    ...
    assert_mock(@mock_db)
    assert_equal(123_456_789_123, max_id)
  end
  ...
  def test_get_file
    id = 123
    sql = 'SELECT * FROM cfs_files WHERE id=$1'
    file_exp = { 'id' => '123', 'name' => 'test', 'size' => 123_456, 'md5_sum' => '12345678901234567890123456789012', 'cfs_directory_id' => 1 }
  ...
  def test_get_files_in_batches
    ...
    id_iterator = 3
    sql = 'SELECT * FROM cfs_files WHERE id>$1 AND  id<=$2'
    files_ret = [{ 'id' => '1', 'name' => 'test', 'size' => 123_456, 'md5_sum' => '12345678901234567890123456789012', 'cfs_directory_id' => 1 },
                 { 'id' => '2', 'name' => 'test1', 'size' => 234_567, 'md5_sum' => '23456789012345678901234567890123', 'cfs_directory_id' => 1 },
                 { 'id' => '3', 'name' => 'test2', 'size' => 345_678, 'md5_sum' => '34567890123456789012345678901234', 'cfs_directory_id' => 2 }]
    @mock_db.expect(:exec_params, files_ret, [sql, [{ value: id.to_s }, { value: id_iterator.to_s }]])
    file_dirs_act, medusa_files_act, size_act = @batch_restore_files.get_files_in_batches(id, id_iterator)

    m_file1 = MedusaFile.new('test', '1', 1, '12345678901234567890123456789012')
    m_file2 = MedusaFile.new('test1', '2', 1, '23456789012345678901234567890123')
    m_file3 = MedusaFile.new('test2', '3', 2, '34567890123456789012345678901234')
    medusa_files_exp = [m_file1, m_file2, m_file3]
    file_dirs_exp = [1, 2]
    size_exp = 123_456 + 234_567 + 345_678
  ...
  def test_generate_manifest
    ...
    dir_path1 = '1/2/3/'
    dir_path2 = '4/5/6/'
    directories = { '1' => dir_path1, '2' => dir_path2 }
    id1 = '123'
    dir1 = 1
    name1 = 'test'
    checksum1 = '12345678901234567890123456789012'
    key1 = dir_path1 + name1
    id2 = '345'
    dir2 = 2
    name2 = 'test1'
    checksum2 = '23456789012345678901234567890123'
    key2 = dir_path2 + name2
    id3 = '456'
    dir3 = 1
    name3 = 'test3'
    key3 = dir_path1 + name3
    checksum3 = '34567890123456789012345678901234'
    medusa_item1 = MedusaFile.new(name1, id1, dir1, checksum1)
    medusa_item2 = MedusaFile.new(name2, id2, dir2, checksum2)
    medusa_item3 = MedusaFile.new(name3, id3, dir3, checksum3)
    medusa_files = [medusa_item1, medusa_item2, medusa_item3]
    batch_hash1 = {
      Settings.aws.dynamodb.s3_key => key1,
      Settings.aws.dynamodb.file_id => id1,
      Settings.aws.dynamodb.initial_checksum => checksum1,
      Settings.aws.dynamodb.restoration_status => Settings.aws.dynamodb.requested,
      Settings.aws.dynamodb.last_updated => Time.new(1).getutc.iso8601(3)
    }
    batch_hash2 = {
      Settings.aws.dynamodb.s3_key => key2,
      Settings.aws.dynamodb.file_id => id2,
      Settings.aws.dynamodb.initial_checksum => checksum2,
      Settings.aws.dynamodb.restoration_status => Settings.aws.dynamodb.requested,
      Settings.aws.dynamodb.last_updated => Time.new(1).getutc.iso8601(3)
    }
    batch_hash3 = {
      Settings.aws.dynamodb.s3_key => key3,
      Settings.aws.dynamodb.file_id => id3,
      Settings.aws.dynamodb.initial_checksum => checksum3,
      Settings.aws.dynamodb.restoration_status => Settings.aws.dynamodb.requested,
      Settings.aws.dynamodb.last_updated => Time.new(1).getutc.iso8601(3)
    }
    batch_exp = [batch_hash1, batch_hash2, batch_hash3]
    keys = [key1, key2, key3]
  ...
  def test_generate_manifest_escapes_special_characters
    ...
    name1_esc = '%23test'
    checksum1 = '12345678901234567890123456789012'
    key1 = dir_path1 + name1_esc
    id2 = '345'
    dir2 = 2
    name2 = '@test1'
    name2_esc = '%40test1'
    checksum2 = '23456789012345678901234567890123'
    key2 = dir_path2 + name2_esc
    id3 = '456'
    dir3 = 1
    name3 = '=test3'
    name3_esc = '%3Dtest3'
    key3 = dir_path1 + name3_esc
    checksum3 = '34567890123456789012345678901234'
    medusa_item1 = MedusaFile.new(name1, id1, dir1, checksum1)
    medusa_item2 = MedusaFile.new(name2, id2, dir2, checksum2)
    medusa_item3 = MedusaFile.new(name3, id3, dir3, checksum3)
  ...
  def test_get_batch_from_list
    ...
    checksum3 = '34567890123456789012345678901234'
    list = [id1, id2, id3]
    ret_val1 = [{ 'id' => id1, 'cfs_directory_id' => dir1, 'name' => name1, 'size' => size1, 'md5_sum' => checksum1 }]
    sql = 'SELECT * FROM cfs_files WHERE id=$1'
    @mock_db.expect(:exec_params, ret_val1, [sql, [{ value: id1.to_s }]])
    ret_val2 = [{ 'id' => id2, 'cfs_directory_id' => dir2, 'name' => name2, 'size' => size2, 'md5_sum' => checksum2 }]
    @mock_db.expect(:exec_params, ret_val2, [sql, [{ value: id2.to_s }]])
    ret_val3 = [{ 'id' => id3, 'cfs_directory_id' => dir3, 'name' => name3, 'size' => size3, 'md5_sum' => checksum3 }]
    @mock_db.expect(:exec_params, ret_val3, [sql, [{ value: id3.to_s }]])
    medusa_item1 = MedusaFile.new(name1, id1, dir1, checksum1)
    medusa_item2 = MedusaFile.new(name2, id2, dir2, checksum2)
    medusa_item3 = MedusaFile.new(name3, id3, dir3, checksum3)
    medusa_files_exp = [medusa_item1, medusa_item2, medusa_item3]
    file_dirs_exp = [1, 2]
    size_exp = size1 + size2 + size3
